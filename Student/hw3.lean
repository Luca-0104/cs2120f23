...
#eval dec2 0    -- expect 0
#eval dec2 1    -- expect 0
#eval dec2 2    -- expect 0
#eval dec2 3    -- expect 1
...
| _, List.nil => List.nil
| f, List.cons h t => f h :: l2l f t

#eval l2l Nat.succ [0, 1, 2]
#eval l2l List.length [[], [123, 234], [1, 2, 3]]
#eval l2l String.length ["asdf", "wergwerg", "123"]

-- lecture notes
def l2l_ans {α β : Type} : (α → β) → (List α) → List β
| _, List.nil => List.nil
| f, (h::t) => (f h)::(l2l f t)

#eval l2l_ans Nat.succ [0, 2, 4]
...
| 0 => PRFV.undefined
| (Nat.succ m) => PRFV.defined m

-- lecture notes
def dec_ans : Nat → PRFV Nat
| 0 => PRFV.undefined
| n' + 1 => PRFV.defined n'
