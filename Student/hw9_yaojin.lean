...
def rotateNTimes : ℕ → Rotation → Rotation
| 0,      _ => r0
| (n+1),  r => (rotateNTimes n r) + r

theorem nsmul_zero_rot : ∀ (a : Rotation), rotateNTimes 0 a = 0 :=
by 
  intros a
  cases a
  repeat { rfl }

theorem nsmul_succ_rot : ∀ (n : ℕ) (x : Rotation), rotateNTimes (n + 1) x = rotateNTimes n x + x :=
by 
  intros n x
  cases n 
  repeat {
...
instance : AddMonoid Rotation := {
  ...
  nsmul:= rotateNTimes,
  nsmul_zero:= nsmul_zero_rot
  nsmul_succ:=nsmul_succ_rot
}
...
-/
theorem sub_eq_add_neg_rot : ∀ (a b : Rotation), a - b = a + -b :=
by
  intros a b
  cases a
  repeat {
...
def zsmul_Rotation : ℤ → Rotation → Rotation
| (Int.ofNat n), r => n • r
| (Int.negSucc n), r => -((n+1) • r)

theorem zsmul_zero'_rot : ∀ (a : Rotation), zsmul_Rotation 0 a = 0 :=
by
  intros a
  cases a
  repeat { rfl }
  
theorem zsmul_succ'_rot : ∀ (n : ℕ) (a : Rotation), zsmul_Rotation (Int.ofNat (Nat.succ n)) a = zsmul_Rotation (Int.ofNat n) a + a :=
by
  intros n a
  cases n
  repeat {
...
theorem zsmul_neg'_rot : ∀ (n : ℕ) (a : Rotation), zsmul_Rotation (Int.negSucc n) a = -zsmul_Rotation (↑(Nat.succ n)) a :=
by
  intros n a
  cases n
  repeat {
...
instance : SubNegMonoid Rotation := {
  sub_eq_add_neg := sub_eq_add_neg_rot,
  zsmul:=zsmul_Rotation,
  zsmul_zero' := zsmul_zero'_rot,  
  zsmul_succ' := zsmul_succ'_rot,  
  zsmul_neg' := zsmul_neg'_rot    
}
