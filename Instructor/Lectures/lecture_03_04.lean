...
def is_enve (n : Nat) : Bool := n%2 = 0
#check String.length

def is_even_len : String → Bool := comp' _ Nat _ is_enve String.length
...
def is_even_len' := comp is_enve String.length
def is_even_len'' := is_enve ∘ String.length



def comp''' {α : Type} : (α → α) → (α → α)
| f => fun a => f (f a)
